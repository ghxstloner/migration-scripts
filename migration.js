...
async function migrarNiveles(connection, data) {
    ...
    for (const row of data) {
        ...
        if (grp && !nivel5Set.has(grp)) {
            ...
            await connection.execute(
                'INSERT INTO nomnivel5 (codorg, descrip, gerencia, descripcion_corta) VALUES (?, ?, ?, ?)',
                [counter5, grp, gerencia, descripcionCorta]
            );
 ...
 async function insertarPersonal(connection, data) {
    ...
    const insertTempQuery = `
        INSERT INTO temp_personal (
            ...
            tipemp, fecharetiro, tipo_funcionario
        ) VALUES ?
    `;
...
async function main() {
    ...
    try {
        ...
        const totalTasks = 10; // Incrementado en 1 para incluir la estructura organizacional
        ...
        try {
            ...
            progressBar.update(4, { currentTask: 'Migrando MEF...' });
            await migrarMEF(connection, dataPersonal);
 
            progressBar.update(5, { currentTask: 'Migrando Deloitte...' });
            await migrarDeloitte(connection, dataPersonal);
 
            progressBar.update(6, { currentTask: 'Migrando Niveles de Personal...' });
            await migrarNivelesPersonal(connection, dataPersonal);
 
            progressBar.update(7, { currentTask: 'Migrando Centro de Costos...' });
            await migrarCentroCostos(connection, dataPersonal);
 
            progressBar.update(8, { currentTask: 'Migrando Tablas Generales...' });
            await migrarTablasGenerales(connection, dataPersonal);

            progressBar.update(9, { currentTask: 'Migrando Puestos...' });
            await migrarPuestos(connection, dataPersonal);
 
            progressBar.update(10, { currentTask: 'Migrando Familiares...' });
